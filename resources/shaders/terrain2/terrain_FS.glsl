...
void main() {
    vec3 normal = normalize(texture(normalMap, fTexCoord).rbg);
    ...
    for (uint i = 0; i < MAX_MATERIALS; ++i) {
        uint msk = (1 << i);
        if ((splatMask & msk) > 0) {
            materialColor += texture(materials[i].diffuseMap, fTexCoord * materials[i].horizontalScaling);
            total = total + 1.0f;
        }
    }
    materialColor /= total;
    ...
    if (dist < tbnRange - 50) {
        ...
        for (uint i = 0; i < MAX_MATERIALS; ++i) {
            uint msk = (1 << i);
            if ((splatMask & msk) > 0) {
                vec3 materialNormal = texture(materials[i].normalMap, fTexCoord * materials[i].horizontalScaling).rbg;
