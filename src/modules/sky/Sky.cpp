...
namespace mod::sky {
    ...
    void Sky::Setup() {
        // Build mesh
        vd::model::MeshPtr pMesh = std::make_shared<vd::model::Mesh>(vd::gapi::AttributeTypeVec ({
            vd::gapi::AttributeType::FLOAT_3,
            vd::gapi::AttributeType::FLOAT_3,
        ...
        for (int i = 0; i < 24; i += 3) {
            vertices.emplace_back(std::vector<float>({
                    kSkyboxVertices[i], kSkyboxVertices[i + 1], kSkyboxVertices[i + 2],
                    0.0f, 1.0f, 0.0f,
                    kSkyboxVertices[i], kSkyboxVertices[i + 1]
            }));
        }

        pMesh->Assign(vd::gapi::DataFragmentation::eAsTriangles, vertices, kSkyboxIndices);
